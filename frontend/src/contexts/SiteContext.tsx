...
const topMenus: Menu[] = normalizeTopMenus();
...
export function SiteProvider({ children }: { children: ReactNode }) {
  ...
  const [layout, setLayout] = useState(() => normalizeLayout(staticLayout || fallback));
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoading(true);
    getLayoutConfig().then((value) => active && setLayout(normalizeLayout(value))).catch(() => undefined).finally(() => active && setLoading(false));
    return () => { active = false; };
  }, [nonce, staticLayout]);
  useEffect(() => {
    let active = true;
...
function normalizeLayout(layout: LayoutConfig): LayoutConfig {
  return { ...layout, topMenus: normalizeTopMenus(layout.topMenus) };
}
