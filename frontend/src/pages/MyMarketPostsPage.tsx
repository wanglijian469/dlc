...
import { Bell, Factory, Gavel, Package, Plus, UserRound } from "lucide-react";
...
export function MyMarketPostsPage() {
  ...
  return <PageFrame title="我的" subtitle={username ? `${username} · ${role === "vendor" ? "厂商账号" : "采购商账号"}` : "管理账号资料和供求信息"}>
    ...
    {role === "buyer" && <div className="account-shortcuts"><Link to="/account/auctions"><Gavel size={20} /><span><strong>我的采购竞价</strong><small>发布、取消与定标</small></span></Link><Link to="/account/notifications"><Bell size={20} /><span><strong>站内消息</strong><small>报价与排名提醒</small></span></Link><Link to="/auctions"><Gavel size={20} /><span><strong>竞价大厅</strong><small>查看公开采购</small></span></Link></div>}
