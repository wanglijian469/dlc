...
export function ProductDetailPage() {
  ...
  return <PageFrame title={product.name} subtitle={product.category?.name || "农机配件产品目录"}>
    <section className="product-showcase">
      <div className="product-main-media"><IndustryCover className="product-detail-fallback" iconSize={86} kind={getProductIndustryKind(product)} />{activeImage && <img alt={product.name} src={activeImage} onError={(event) => { event.currentTarget.style.display = "none"; }} />}{gallery.length > 1 && <div aria-label="产品图片选择" className="product-thumbnails" role="list">{gallery.map((src, index) => <button aria-label={`查看第 ${index + 1} 张产品图片`} aria-pressed={src === activeImage} key={src} onClick={() => setActiveImage(src)} type="button"><img alt="" src={src} /></button>)}</div>}</div>
