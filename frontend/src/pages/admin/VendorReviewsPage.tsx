...
const comparedFields: Array<{ key: keyof Vendor; label: string }> = [
  ...
  { key: "serviceAdvantages", label: "服务优势" }, { key: "mainProducts", label: "主营产品" }, { key: "description", label: "公司简介" },
  { key: "establishedYear", label: "成立年份" }, { key: "factoryArea", label: "厂房面积" }, { key: "employeeCount", label: "员工规模" }, { key: "annualCapacity", label: "年产能" },
  { key: "equipment", label: "主要设备" }, { key: "certifications", label: "认证资质" }, { key: "websiteUrl", label: "官网" }, { key: "contactName", label: "联系人" }, { key: "contactNamePublic", label: "联系人公开展示" }, { key: "phone", label: "电话" }, { key: "phonePublic", label: "电话公开展示" }, { key: "wechat", label: "微信" }, { key: "wechatQrCode", label: "微信二维码" }, { key: "wechatPublic", label: "微信公开展示" },
  ...
  return (
    <article className="admin-panel vendor-review-card">
      ...
      <div className="review-diff-table">
        ...
        {(changes.length ? changes : comparedFields.slice(0, 3)).map(({ key, label }) => <div className="diff-row" key={key}><strong>{label}</strong><span>{key === "wechatQrCode" && row.vendor?.wechatQrCode ? <ProtectedMediaImage alt="当前微信二维码" assetId={row.vendor.wechatQrCodeAssetId} src={row.vendor.wechatQrCode} /> : formatValue(row.vendor?.[key])}</span><span>{key === "wechatQrCode" && row.draft?.wechatQrCode ? <ProtectedMediaImage alt="提交的微信二维码" assetId={row.draft.wechatQrCodeAssetId} src={row.draft.wechatQrCode} /> : formatValue(row.draft?.[key])}</span></div>)}
      </div>
