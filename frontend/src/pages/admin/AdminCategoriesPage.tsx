...
import { createResource, deleteResource, listResource, saveRevision, submitRevision, updateResource } from "../../api/admin";
...
export function AdminCategoriesPage() {
  ...
  const submit = (event: FormEvent) => {
    ...
    const request = async () => {
      if (isEditor) {
        if (!editingId) throw new Error("编辑角色只能提交现有分类的修订");
        const baseVersion = rows.find((row) => row.id === editingId)?.contentVersion || 1;
        const revision = await saveRevision({ resourceType: "category", resourceId: editingId, baseVersion, snapshot: payload });
        await submitRevision(revision.id);
        return;
      }
      await (editingId ? updateResource("categories", editingId, payload) : createResource("categories", payload));
    };
    void request().then(() => {
      setEditorOpen(false);
      setMessage(isEditor ? "分类修订已保存并送审" : editingId ? "分类修改成功" : "分类创建成功，刷新前台后菜单将自动更新");
      void load();
  ...
  return (
    <AdminLayout title="配件分类">
      ...
      <div className="admin-toolbar admin-panel compact category-toolbar">
        ...
        {!isEditor && <div className="category-admin-actions">
          ...
          <tbody>
            {displayedRows.map((category) => {
              ...
              return <tr className={isChild ? "category-child-row" : ""} key={category.id}>
                ...
                <td data-label="操作"><div className="category-row-actions">{!isEditor && !isChild && <button type="button" onClick={() => openCreate("child", category.id)}>新增子分类</button>}<button type="button" onClick={() => openEdit(category)}>编辑</button>{!isEditor && <button type="button" onClick={() => { if (!window.confirm(`确认删除分类“${category.name}”吗？`)) return; void deleteResource("categories", category.id).then(() => { setMessage("分类删除成功"); void load(); }).catch((error) => setMessage(getApiErrorMessage(error, "分类删除失败"))); }}>删除</button>}</div></td>
              </tr>;
            })}
      ...
      {editorOpen && <AdminModal label={`${editingId ? "编辑" : "新增"}配件分类`} onClose={() => setEditorOpen(false)}>
        ...
        <form className="admin-form admin-grouped-form" onSubmit={submit}><fieldset><legend>分类信息</legend><div className="admin-field-grid">
          ...
          <label>短拼音 URL 标识<input placeholder="例如：fadongji" value={form.slug} onChange={(event) => setForm({ ...form, slug: event.target.value })} /></label>
