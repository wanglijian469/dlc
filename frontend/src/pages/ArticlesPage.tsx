...
export function ArticlePage({ slug }: { slug: string }) {
  ...
  return <PageFrame title={article.title} subtitle={article.summary} breadcrumbs={[{ label: "行业指南", path: "/guides" }]}><article className="cms-page-content"><p className="article-meta">{article.authorName || "大陆农机配件"}{article.publishedAt ? ` · ${article.publishedAt.slice(0, 10)}` : ""}</p>{article.coverImage && <img alt={article.title} className="article-cover" src={article.coverImage} />}{article.blocks?.length ? article.blocks.map((block, index) => <ContentBlockView block={block} key={`${block.type}-${index}`} />) : article.content ? <section className="content-text-block"><p>{article.content}</p></section> : <EmptyState text="该文章暂未填写正文内容" />}<div className="article-related-links">{article.relatedCategoryId && <Link to={`/products?categoryId=${article.relatedCategoryId}`}>查看相关配件分类</Link>}{article.relatedProductId && <Link to={`/products/${article.relatedProductId}`}>查看相关产品</Link>}{article.relatedVendorId && <Link to={`/v/${article.relatedVendorId}`}>查看相关厂商</Link>}</div><Link className="primary-btn" to="/join">厂商入驻</Link></article></PageFrame>;
}
