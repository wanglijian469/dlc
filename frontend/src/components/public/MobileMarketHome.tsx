...
import type { HomePayload, MarketPost, Product } from "../../types/api";

export function MobileMarketHome({ home }: { home: HomePayload }) {
  ...
  useEffect(() => {
    ...
    void listMarketPosts({ pageSize: 6 }).then((result) => setPosts(result.items)).catch(() => undefined);
  }, []);
  ...
  return <div className="mobile-market-home">
    ...
    <nav className="mobile-market-shortcuts"><Link to="/products"><Grid2X2 /><span>找配件</span></Link><Link to="/vendors"><Factory /><span>找厂家</span></Link><Link to="/service"><Wrench /><span>加工服务</span></Link><Link to="/publish"><span className="shortcut-publish">+</span><span>发供求</span></Link></nav>
    {posts.length > 0 && <section className="mobile-feed-section"><header><h2>最新供求</h2><Link to="/purchase">查看全部</Link></header><div className="mobile-feed-strip">{posts.map((post) => <Link key={post.id} to={`/purchase/${post.id}`}>{post.images[0] ? <img alt="" src={post.images[0]} /> : <span className={`feed-type ${post.type}`}>{post.type === "demand" ? "求购" : "供应"}</span>}<strong>{post.title}</strong><small>{[post.province, post.city].filter(Boolean).join(" · ") || "全国"}</small></Link>)}</div></section>}
