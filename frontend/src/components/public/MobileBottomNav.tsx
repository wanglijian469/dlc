import { ClipboardList, Grid2X2, Home, Factory, UserRound } from "lucide-react";
import { Link, useLocation } from "react-router-dom";
import type { Menu } from "../../types/api";

const iconMap = { home: Home, grid: Grid2X2, factory: Factory, clipboard: ClipboardList, user: UserRound };
const items: Menu[] = [
  ...
  return (
    <nav aria-label="移动端主导航" className="mobile-bottom-nav">
      {items.map((item) => {
        ...
        return <Link className={`${active ? "active" : ""} ${path === "/publish" ? "publish-item" : ""}`} key={item.id} to={path === "/account/posts" && localStorage.getItem("cms_role") === "vendor" ? "/admin/vendor-workspace" : path}><Icon size={20} /><span>{item.name}</span></Link>;
      })}
    </nav>
  );
}
