...
export function ProtectedMediaImage({ assetId, src, alt, className }: { assetId?: number; src: string; alt: string; className?: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/admin/media/${assetId}`, { headers: { Authorization: `Bearer ${localStorage.getItem("admin_token") || ""}` }, signal: controller.signal }).then((response) => { if (!response.ok) throw new Error(); return response.blob(); }).then((blob) => { objectURL = URL.createObjectURL(blob); setResolved(objectURL); }).catch(() => setResolved(""));
