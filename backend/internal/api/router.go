...
func RegisterPublicRoutesWithAuth(router *gin.Engine, db *gorm.DB, secret string) {
	...
	api.GET("/vendors/:id/posts", handler.VendorPosts)
	api.GET("/vendors/:id", handler.VendorDetail)
	api.GET("/products", handler.Products)
	api.GET("/showrooms/:slug/products", handler.ShowroomProducts)
	api.GET("/showrooms/:slug/products/:supplierId", handler.ShowroomProduct)
...
func RegisterMarketplaceRoutes(router *gin.Engine, db *gorm.DB, cfg config.Config) {
	...
	appAuth.POST("/login", handler.AppLogin)
	appAuth.POST("/refresh", handler.AppRefresh)
	appProtected := appAuth.Group("")
	...
	auctions.GET("", handler.ListPublicAuctions)
	auctions.GET("/:id", handler.PublicAuctionDetail)
	auctionBid := auctions.Group("")
	auctionBid.Use(CMSAuth(db, cfg.AuthSecret), RequireCSRF(), RequireRole("vendor"))
	auctionBid.POST("/:id/bids", handler.PlaceAuctionBid)
	...
	account.GET("/auctions/:id", handler.OwnAuctionDetail)
	account.POST("/auctions", handler.CreateOwnAuction)
	account.PUT("/auctions/:id", handler.UpdateOwnAuction)
	account.POST("/auctions/:id/publish", handler.PublishOwnAuction)
	account.POST("/auctions/:id/cancel", handler.CancelOwnAuction)
	account.POST("/auctions/:id/award", handler.AwardOwnAuction)
	account.GET("/notifications", handler.ListNotifications)
	account.PUT("/notifications/read-all", handler.ReadAllNotifications)
	account.PUT("/notifications/:id/read", handler.ReadNotification)

	media := router.Group("/api/v1")
	media.Use(CMSAuth(db, cfg.AuthSecret), RequireCSRF(), RequireAnyRole("buyer", "vendor"))
...
func registerAdminRoutesWithServices(router *gin.Engine, db *gorm.DB, cfg config.Config, staticPages *StaticPageService, accessProtection *AccessProtectionService, watermarks *WatermarkService, capture *CaptureService) {
	...
	workflowReviewers.PUT("/auctions/:id/status", handler.AdminUpdateAuctionStatus)
	...
	vendorOnly.GET("/vendor-workspace", handler.VendorWorkspace)
	vendorOnly.GET("/vendor-work-drafts", handler.ListWorkDrafts)
	vendorOnly.PUT("/vendor-work-drafts/:key", handler.SaveWorkDraft)
	vendorOnly.DELETE("/vendor-work-drafts/:id", handler.DeleteWorkDraft)
	vendorOnly.POST("/vendor-work-drafts/:id/commit", handler.CommitWorkDraft)
	vendorOnly.PUT("/vendor-products/:id/showroom", handler.UpdateShowroomOrder)
	vendorOnly.GET("/vendor-analytics", AnalyticsHandler{DB: db, Config: cfg}.VendorSummary)
	vendorOnly.POST("/vendor-posts", handler.CreateOwnVendorPost)
	vendorOnly.PUT("/vendor-posts/:id", handler.UpdateOwnVendorPost)
	vendorOnly.POST("/vendor-posts/:id/submit", handler.SubmitOwnVendorPost)
	vendorOnly.POST("/vendor-posts/:id/withdraw", handler.WithdrawOwnVendorPost)

	vendorPostReaders := protected.Group("")
	vendorPostReaders.Use(RequireAnyRole("admin", "reviewer", "vendor"))
	vendorPostReaders.GET("/vendor-posts", handler.ListVendorPostsForRole)
